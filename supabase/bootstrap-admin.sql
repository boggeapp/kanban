-- Execute depois de confirmar o cadastro ou de enviar um convite pelo Supabase.
...
begin
 ...
 select id into target_id from auth.users where lower(email)=lower(target_email) and (email_confirmed_at is not null or invited_at is not null);
 if target_id is null then raise exception 'Conta não encontrada, não confirmada ou sem convite administrativo'; end if;
